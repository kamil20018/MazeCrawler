...
GamePlay::GamePlay(std::shared_ptr<Context>& context, std::shared_ptr<Hero> passedHero) :
	context(context), state(Turn::HERO_TURN), hero(passedHero) {
	level += 1;
	firstInit(false);
}
...
void GamePlay::firstInit(bool makeHero) {
...
void GamePlay::Update() {
	...
	if (heroPos == this->exit) {
		this->context->states->AddState(std::make_unique<GamePlay>(this->context, this->hero));
	}
